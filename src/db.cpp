...
mysql::mysql(config * conf) : u_active(false), t_active(false), p_active(false), s_active(false), tok_active(false) {
	...
	try {
		...
		cout << "Failed to connect to MySQL (" << er.what() << ')' << endl;
		return;
	}

	if (!readonly) {
		cout << "Clearing xbt_files_users and resetting peer counts...";
		cout.flush();
		clear_peer_data();
		cout << "done" << endl;
	}
...
void mysql::clear_peer_data() {
	try {
		...
		if (!query.exec()) {
			cerr << "Unable to truncate xbt_files_users!" << endl;
		}
		query = conn.query("UPDATE torrents SET Seeders = 0, Leechers = 0;");
		if (!query.exec()) {
			cerr << "Unable to reset seeder and leecher count!" << endl;
		}
	} catch (const mysqlpp::BadQuery &er) {
		cerr << "Query error in clear_peer_data: " << er.what() << endl;
	} catch (const mysqlpp::Exception &er) {
		cerr << "Query error in clear_peer_data: " << er.what() << endl;
	}
}
...
void mysql::load_torrents(torrent_list &torrents) {
	...
	try {
		...
		unordered_set<string> cur_keys;
		size_t num_rows = res.num_rows();
		lock_guard<mutex> tl_lock(torrent_list_mutex);
		if (torrents.size() == 0) {
		...
		for (size_t i = 0; i < num_rows; i++) {
			...
			torrent tmp_tor;
			auto it = torrents.insert(pair<string, torrent>(info_hash, tmp_tor));
		...
		cerr << "Query error in load_torrents: " << er.what() << endl;
		return;
	}
	cout << "Loaded " << torrents.size() << " torrents" << endl;
...
void mysql::load_users(user_list &users) {
	...
	try {
		...
		size_t num_rows = res.num_rows();
		unordered_set<string> cur_keys;
		lock_guard<mutex> ul_lock(user_list_mutex);
		if (users.size() == 0) {
		...
		for (size_t i = 0; i < num_rows; i++) {
			string passkey(res[i][2]);
			bool protect_ip = res[i][3];
			user_ptr tmp_user = make_shared<user>(res[i][0], res[i][1], protect_ip);
			auto it = users.insert(pair<string, user_ptr>(passkey, tmp_user));
			if (!it.second) {
		...
		cerr << "Query error in load_users: " << er.what() << endl;
		return;
	}
...
void mysql::load_tokens(torrent_list &torrents) {
	...
	try {
		...
		cerr << "Query error in load_tokens: " << er.what() << endl;
		return;
	}
	cout << "Loaded " << token_count << " tokens" << endl;
}


void mysql::load_whitelist(vector<string> &whitelist) {
	mysqlpp::Query query = conn.query("SELECT peer_id FROM xbt_client_whitelist;");
	try {
		mysqlpp::StoreQueryResult res = query.store();
		size_t num_rows = res.num_rows();
		lock_guard<mutex> wl_lock(whitelist_mutex);
		whitelist.clear();
		for (size_t i = 0; i<num_rows; i++) {
			string peer_id;
		...
		cerr << "Query error in load_whitelist: " << er.what() << endl;
		return;
	}
	if (whitelist.size() == 0) {
		cout << "Assuming no whitelist desired, disabling" << endl;
	} else {
		cout << "Loaded " << whitelist.size() << " clients into the whitelist" << endl;
	}
}

void mysql::record_token(const string &record) {
...
void mysql::record_user(const string &record) {
...
void mysql::record_torrent(const string &record) {
	lock_guard<mutex> tb_lock(torrent_buffer_lock);
...
void mysql::record_peer(const string &record, const string &ip, const string &peer_id, const string &useragent) {
...
void mysql::record_peer(const string &record, const string &peer_id) {
...
void mysql::record_snatch(const string &record, const string &ip) {
...
void mysql::flush_users() {
	...
	string sql;
	lock_guard<mutex> uq_lock(user_queue_lock);
	size_t qsize = user_queue.size();
	if (verbose_flush || qsize > 0) {
		cout << "User flush queue size: " << qsize << ", next query length: " << user_queue.front().size() << endl;
	}
	if (update_user_buffer == "") {
		return;
	}
	...
	update_user_buffer.clear();
	if (u_active == false) {
		thread thread(&mysql::do_flush_users, this);
		thread.detach();
	}
}
...
void mysql::flush_torrents() {
	...
	string sql;
	lock_guard<mutex> tq_lock(torrent_queue_lock);
	size_t qsize = torrent_queue.size();
	if (verbose_flush || qsize > 0) {
		cout << "Torrent flush queue size: " << qsize << ", next query length: " << torrent_queue.front().size() << endl;
	}
	if (update_torrent_buffer == "") {
		return;
	}
	...
	torrent_queue.push(sql);
	if (t_active == false) {
		thread thread(&mysql::do_flush_torrents, this);
		thread.detach();
	}
}
...
void mysql::flush_snatches() {
	...
	string sql;
	lock_guard<mutex> sq_lock(snatch_queue_lock);
	size_t qsize = snatch_queue.size();
	if (verbose_flush || qsize > 0) {
		cout << "Snatch flush queue size: " << qsize << ", next query length: " << snatch_queue.front().size() << endl;
	}
	if (update_snatch_buffer == "" ) {
		return;
	}
	...
	update_snatch_buffer.clear();
	if (s_active == false) {
		thread thread(&mysql::do_flush_snatches, this);
		thread.detach();
	}
}
...
void mysql::flush_peers() {
	...
	string sql;
	lock_guard<mutex> pq_lock(peer_queue_lock);
	size_t qsize = peer_queue.size();
	if (verbose_flush || qsize > 0) {
		cout << "Peer flush queue size: " << qsize << ", next query length: " << peer_queue.front().size() << endl;
	}
	...
	if (update_heavy_peer_buffer != "") {
		...
		sql = "INSERT INTO xbt_files_users (uid,fid,active,uploaded,downloaded,upspeed,downspeed,remaining,corrupt," +
			string("timespent,announced,ip,peer_id,useragent,mtime) VALUES ") + update_heavy_peer_buffer +
	...
	if (p_active == false) {
		thread thread(&mysql::do_flush_peers, this);
		thread.detach();
	}
}
...
void mysql::flush_tokens() {
	...
	string sql;
	lock_guard<mutex> tq_lock(token_queue_lock);
	size_t qsize = token_queue.size();
	if (verbose_flush || qsize > 0) {
		cout << "Token flush queue size: " << qsize << ", next query length: " << token_queue.front().size() << endl;
	}
	if (update_token_buffer == "") {
		return;
	}
	...
	update_token_buffer.clear();
	if (tok_active == false) {
		thread thread(&mysql::do_flush_tokens, this);
		thread.detach();
	}
}
...
void mysql::do_flush_users() {
	...
	try {
		...
		while (user_queue.size() > 0) {
			try {
				string sql = user_queue.front();
				mysqlpp::Query query = c.query(sql);
				if (!query.exec()) {
					cout << "User flush failed (" << user_queue.size() << " remain)" << endl;
					sleep(3);
					continue;
				} else {
					lock_guard<mutex> uq_lock(user_queue_lock);
				...
				cerr << "Query error: " << er.what() << " in flush users with a qlength: " << user_queue.front().size() << " queue size: " << user_queue.size() << endl;
				sleep(3);
				continue;
			} catch (const mysqlpp::Exception &er) {
				cerr << "Query error: " << er.what() << " in flush users with a qlength: " << user_queue.front().size() <<  " queue size: " << user_queue.size() << endl;
		...
		cerr << "MySQL error in flush_users: " << er.what() << endl;
	}
	u_active = false;
}
...
void mysql::do_flush_torrents() {
	...
	try {
		...
		while (torrent_queue.size() > 0) {
			try {
				string sql = torrent_queue.front();
				if (sql == "") {
					torrent_queue.pop();
					continue;
				}
				mysqlpp::Query query = c.query(sql);
				if (!query.exec()) {
					cout << "Torrent flush failed (" << torrent_queue.size() << " remain)" << endl;
					sleep(3);
					continue;
				} else {
					lock_guard<mutex> tq_lock(torrent_queue_lock);
				...
				cerr << "Query error: " << er.what() << " in flush torrents with a qlength: " << torrent_queue.front().size() << " queue size: " << torrent_queue.size() << endl;
				sleep(3);
				continue;
			} catch (const mysqlpp::Exception &er) {
				cerr << "Query error: " << er.what() << " in flush torrents with a qlength: " << torrent_queue.front().size() << " queue size: " << torrent_queue.size() << endl;
		...
		cerr << "MySQL error in flush_torrents: " << er.what() << endl;
	}
	t_active = false;
}
...
void mysql::do_flush_peers() {
	...
	try {
		...
		while (peer_queue.size() > 0) {
			try {
				string sql = peer_queue.front();
				mysqlpp::Query query = c.query(sql);
				if (!query.exec()) {
					cout << "Peer flush failed (" << peer_queue.size() << " remain)" << endl;
					sleep(3);
					continue;
				} else {
					lock_guard<mutex> pq_lock(peer_queue_lock);
				...
				cerr << "Query error: " << er.what() << " in flush peers with a qlength: " << peer_queue.front().size() << " queue size: " << peer_queue.size() << endl;
				sleep(3);
				continue;
			} catch (const mysqlpp::Exception &er) {
				cerr << "Query error: " << er.what() << " in flush peers with a qlength: " << peer_queue.front().size() << " queue size: " << peer_queue.size() << endl;
		...
		cerr << "MySQL error in flush_peers: " << er.what() << endl;
	}
	p_active = false;
}
...
void mysql::do_flush_snatches() {
	...
	try {
		...
		while (snatch_queue.size() > 0) {
			try {
				string sql = snatch_queue.front();
				mysqlpp::Query query = c.query(sql);
				if (!query.exec()) {
					cout << "Snatch flush failed (" << snatch_queue.size() << " remain)" << endl;
					sleep(3);
					continue;
				} else {
					lock_guard<mutex> sq_lock(snatch_queue_lock);
				...
				cerr << "Query error: " << er.what() << " in flush snatches with a qlength: " << snatch_queue.front().size() << " queue size: " << snatch_queue.size() << endl;
				sleep(3);
				continue;
			} catch (const mysqlpp::Exception &er) {
				cerr << "Query error: " << er.what() << " in flush snatches with a qlength: " << snatch_queue.front().size() << " queue size: " << snatch_queue.size() << endl;
		...
		cerr << "MySQL error in flush_snatches: " << er.what() << endl;
	}
	s_active = false;
}
...
void mysql::do_flush_tokens() {
	...
	try {
		...
		while (token_queue.size() > 0) {
			try {
				string sql = token_queue.front();
				mysqlpp::Query query = c.query(sql);
				if (!query.exec()) {
					cout << "Token flush failed (" << token_queue.size() << " remain)" << endl;
					sleep(3);
					continue;
				} else {
					lock_guard<mutex> tq_lock(token_queue_lock);
				...
				cerr << "Query error: " << er.what() << " in flush tokens with a qlength: " << token_queue.front().size() << " queue size: " << token_queue.size() << endl;
				sleep(3);
				continue;
			} catch (const mysqlpp::Exception &er) {
				cerr << "Query error: " << er.what() << " in flush tokens with a qlength: " << token_queue.front().size() << " queue size: " << token_queue.size() << endl;
		...
		cerr << "MySQL error in flush_tokens: " << er.what() << endl;
	}
	tok_active = false;
}
