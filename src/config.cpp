...
string confval::get_str() {
	return str_val;
}

void confval::set(const string &value) {
...
confval * config::get(const string &setting_name) {
	const auto setting = settings.find(setting_name);
	if (setting == settings.end()) {
		cout << "WARNING: Unrecognized setting '" << setting_name << "'" << endl;
		return dummy_setting;
	}
	return &setting->second;
}

bool config::get_bool(const string &setting_name) {
	return get(setting_name)->get_bool();
}

unsigned int config::get_uint(const string &setting_name) {
	return get(setting_name)->get_uint();
}

string config::get_str(const string &setting_name) {
	return get(setting_name)->get_str();
}

void config::set(const string &setting_name, const string &value) {
	get(setting_name)->set(value);
}

void config::load(const string &conf_file_path, istream &conf_file) {
	load(conf_file);
	add("conf_file_path", conf_file_path.c_str());
}

void config::load(istream &conf_file) {
	string line;
...
void config::reload() {
	const string conf_file_path(get_str("conf_file_path"));
	ifstream conf_file(conf_file_path);
	if (conf_file.fail()) {
		cout << "Config file '" << conf_file_path << "' couldn't be opened" << endl;
	} else {
		init();
		load(conf_file);
	}
